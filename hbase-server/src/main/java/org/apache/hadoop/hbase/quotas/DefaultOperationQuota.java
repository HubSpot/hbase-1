...
public class DefaultOperationQuota implements OperationQuota {
  ...
  public void checkScanQuota(ClientProtos.ScanRequest scanRequest, long maxScannerResultSize,
    ...
    for (final QuotaLimiter limiter : limiters) {
      ...
      limiter.checkQuota(0, writeConsumed, 1, readConsumed,
        writeCapacityUnitConsumed, readCapacityUnitConsumed);
      readAvailable = Math.min(readAvailable, limiter.getReadAvailable());
    }

    for (final QuotaLimiter limiter : limiters) {
      limiter.grabQuota(0, writeConsumed, 1, readConsumed,
        writeCapacityUnitConsumed, readCapacityUnitConsumed);
    }
  }
   ...
   */
  protected void updateEstimateConsumeScanQuota(ClientProtos.ScanRequest scanRequest, long maxScannerResultSize, long maxBlockBytesScanned) {
    if (useResultSizeBytes) {
      readConsumed = estimateConsume(OperationType.GET, 1, 1000);
    } else {
      /*
       * Estimating scan workload is more complicated, and if we severely underestimate
       * workloads then throttled clients will exhaust retries too quickly, and could
       * saturate the RPC layer.
       * We have access to the ScanRequest's nextCallSeq number, the maxScannerResultSize,
       * and the maxBlockBytesScanned by every relevant Scanner#next call.
       * With these inputs we can make a more informed estimate about the scan's workload.
       */
      long estimate;
