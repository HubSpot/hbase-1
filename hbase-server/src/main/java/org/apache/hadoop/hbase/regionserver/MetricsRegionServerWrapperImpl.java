  ...
  private static class RegionMetricAggregate {
    ...
    private void aggregate(HRegionServer regionServer,
      ...
      for (HRegion r : regionServer.getOnlineRegionsLocalContext()) {
        Pair<Long, Long> readWriteDeltas = calculateReadWriteDeltas(r, requestsCountCache);
        totalReadRequestsDelta += readWriteDeltas.getFirst();
        totalWriteRequestsDelta += readWriteDeltas.getSecond();
    ...
    private Pair<Long, Long> calculateReadWriteDeltas(HRegion r,
      Map<String, ArrayList<Long>> requestsCountCache) {
      ...
      if (requestsCountCache.containsKey(encodedRegionName)) {
        ...
        return new Pair<>(readRequestsDelta, writeRequestsDelta);
      } else {
        ...
        return new Pair<>(currentReadRequestsCount, currentWriteRequestsCount);
      }
    }

    public void updateRequestsPerSecond(long timeSinceLastRun, long expectedPeriod) {
      requestsPerSecond =
    ...
    synchronized public void run() {
      try {
        ...
        if (timeSinceLastRun > 0) {
          newVal.updateRequestsPerSecond(timeSinceLastRun, period);
        }
