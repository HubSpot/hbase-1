...
public class RegionServerRpcQuotaManager {
   ...
   * This method is specific to scans because estimating a scan's workload is more complicated than
   * estimating the workload of a get/put.
   *
   * @param region               the region where the operation will be performed
   * @param scanRequest          the scan to be estimated against the quota
   * @param maxScannerResultSize
   * @return the OperationQuota
   * @throws RpcThrottlingException if the operation cannot be executed due to quota exceeded.
   */
  public OperationQuota checkScanQuota(final Region region, final ClientProtos.ScanRequest scanRequest,
    ...
    try {
      ...
      LOG.debug(
        "Throttling exception for user=" + ugi.getUserName() + " table=" + table + " scan=" + scanRequest.toString() + ": " + e.getMessage()); // todo rmattingly replace scan string with useful details about it
      throw e;
    }
    return quota;
  }
   ...
   * Check the quota for the current (rpc-context) user. Returns the OperationQuota used to get the
   * available quota and to report the data/usage of the operation.
   * This method does not support scans because estimating a scan's workload is more complicated than
   * estimating the workload of a get/put.
   * @param region the region where the operation will be performed
   ...
   * @throws RpcThrottlingException if the operation cannot be executed due to quota exceeded.
   */
  public OperationQuota checkBatchQuota(final Region region, final OperationQuota.OperationType type)
    throws IOException, RpcThrottlingException {
    switch (type) {
      case GET:
   ...
   * @throws RpcThrottlingException if the operation cannot be executed due to quota exceeded.
   */
  public OperationQuota checkBatchQuota(final Region region, final List<ClientProtos.Action> actions,
   ...
   * @throws RpcThrottlingException if the operation cannot be executed due to quota exceeded.
   */
  private OperationQuota checkBatchQuota(final Region region, final int numWrites, final int numReads) throws IOException, RpcThrottlingException {
    Optional<User> user = RpcServer.getRequestUser();
    UserGroupInformation ugi;
    if (user.isPresent()) {
      ugi = user.get().getUGI();
    ...
    try {
      ...
      LOG.debug(
        "Throttling exception for user=" + ugi.getUserName() + " table=" + table + " numWrites="
          + numWrites + " numReads=" + numReads + ": " + e.getMessage());
      throw e;
    }
    return quota;
  }
