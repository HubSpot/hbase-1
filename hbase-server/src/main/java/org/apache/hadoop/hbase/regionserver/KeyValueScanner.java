...
public interface KeyValueScanner extends Shipper, Closeable {
   ...
   * @param oldestUnexpiredTS the oldest timestamp we are interested in for this query, based on TTL
  ...
  /**
   * Similar to {@link #seek} (or {@link #reseek} if forward is true) but only does a seek operation
   ...
   *
   * @param blockConsumer to be called with block size in bytes, once per block.
   */
  void recordBlockSize(Consumer<HFileBlock> blockConsumer);
