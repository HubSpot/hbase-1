...
public class HFileReplicator implements Closeable {
  ...
  public Void replicate() throws IOException {
    ...
    for (Entry<String, Path> tableStagingDir : tableToSinkStagingDir.entrySet()) {
      ...
      Table sinkTable = this.connection.getTable(sinkTableName);

      // Prepare collection of queue of hfiles to be loaded(replicated)
      Deque<LoadQueueItem> queue = new LinkedList<>();
      loadHFiles.prepareHFileQueue(sinkStagingDir, sinkTable, queue, false);
      ...
      try (RegionLocator locator = connection.getRegionLocator(tableName)) {
        ...
        // data
        loadHFiles.setBulkToken(sinkStagingDir.toString());
        doBulkLoad(loadHFiles, sinkTable, queue, locator, maxRetries);
      } finally {
        cleanup(sinkStagingDir.toString(), sinkTable);
      }
    }
    return null;
  }
