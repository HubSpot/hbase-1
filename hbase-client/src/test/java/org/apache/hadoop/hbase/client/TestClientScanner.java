...
public class TestClientScanner {
    ...
    protected boolean moveToNextRegion() {
      ...
      if (rpcFinishedFired) {
        throw new RuntimeException(
          "Expected nextScanner to only be called once after " + " short-circuit was triggered.");
      }
  ...
  public void testNoResultsHint() throws IOException {
    ...
    Mockito.when(caller.callWithoutRetries(Mockito.any(), Mockito.anyInt()))
  ...
  public void testExceptionsFromReplicasArePropagated() throws IOException {
    ...
    when(clusterConn.locateRegion((TableName) any(), (byte[]) any(), anyBoolean(), anyBoolean(),
      anyInt())).thenReturn(new RegionLocations(null, null, null));
